...
* Построение покрытия: `go test -v -coverprofile=cover.out; go tool cover -html=cover.out -o cover.html`. Для построения покрытия ваш код должен находиться внутри GOPATH
