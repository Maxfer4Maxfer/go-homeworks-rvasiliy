...
func TestACL(t *testing.T) {
	...
	biz := protobuf.NewBizClient(conn)
	adm := protobuf.NewAdminClient(conn)
	...
	for idx, ctx := range contexts {
		_, err = biz.Test(ctx, &protobuf.Nothing{})
		if err == nil {
	...
	// есть доступ
	_, err = biz.Check(getConsumerCtx("biz_user"), &protobuf.Nothing{})
	if err != nil {
		t.Fatalf("ACL fail: unexpected error: %v", err)
	}
	_, err = biz.Check(getConsumerCtx("biz_admin"), &protobuf.Nothing{})
	if err != nil {
		t.Fatalf("ACL fail: unexpected error: %v", err)
	}

	_, err = biz.Test(getConsumerCtx("biz_admin"), &protobuf.Nothing{})
	if err != nil {
		t.Fatalf("ACL fail: unexpected error: %v", err)
	}

	// ACL на методах, которые возвращают поток данных
	logger, err := adm.Logging(getConsumerCtx("unknown"), &protobuf.Nothing{})
	_, err = logger.Recv()
	if err == nil {
		t.Fatalf("ACL 4fail: expected err on disallowed method")
	} else if code := grpc.Code(err); code != codes.Unauthenticated {
		t.Fatalf("ACL 5fail: expected Unauthenticated code, got %v", code)
	}
}
...
func TestLogging(t *testing.T) {
	...
	logStream1, err := adm.Logging(getConsumerCtx("logger"), &protobuf.Nothing{})
	time.Sleep(1 * time.Millisecond)
	logStream2, err := adm.Logging(getConsumerCtx("logger"), &protobuf.Nothing{})

	logData1 := []*protobuf.Event{}
	logData2 := []*protobuf.Event{}
	...
	biz.Check(getConsumerCtx("biz_user"), &protobuf.Nothing{})
	time.Sleep(2 * time.Millisecond)

	biz.Check(getConsumerCtx("biz_admin"), &protobuf.Nothing{})
	time.Sleep(2 * time.Millisecond)

	biz.Test(getConsumerCtx("biz_admin"), &protobuf.Nothing{})
	time.Sleep(2 * time.Millisecond)

	wg.Wait()

	expectedLogData1 := []*protobuf.Event{
	...
	expectedLogData2 := []*protobuf.Event{
		{Timestamp: 0, Consumer: "biz_user", Method: "/main.Biz/Check", Host: ""},
		{Timestamp: 0, Consumer: "biz_admin", Method: "/main.Biz/Check", Host: ""},
		{Timestamp: 0, Consumer: "biz_admin", Method: "/main.Biz/Test", Host: ""},
	}
...
func TestStat(t *testing.T) {
	...
	statStream1, err := adm.Statistics(getConsumerCtx("stat"), &protobuf.StatInterval{IntervalSeconds: 2})
	wait(1)
	statStream2, err := adm.Statistics(getConsumerCtx("stat"), &protobuf.StatInterval{IntervalSeconds: 3})
	...
	biz.Check(getConsumerCtx("biz_user"), &protobuf.Nothing{})
	biz.Add(getConsumerCtx("biz_user"), &protobuf.Nothing{})
	biz.Test(getConsumerCtx("biz_admin"), &protobuf.Nothing{})

	wait(200) // 2 sec

	expectedStat1 := &protobuf.Stat{
		Timestamp: 0,
	...
	biz.Add(getConsumerCtx("biz_admin"), &protobuf.Nothing{})
