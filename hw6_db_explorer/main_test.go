...
// go test -v main_test.go db_explorer.go main.go database.go router.go api.go -coverprofile=cover.out; go tool cover -html=cover.out -o cover.html
// open -a "Safari" cover.html
...
func runCases(t *testing.T, ts *httptest.Server, db *sql.DB, cases []Case) {
	for idx, item := range cases {
		...
		caseName := fmt.Sprintf("case %d: [%s] %s %s", idx+1, item.Method, item.Path, item.Query)
		...
		// fmt.Println("Expectation", item.Result)
